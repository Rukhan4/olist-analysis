...
CREATE OR REPLACE VIEW order_primary_category AS
-- DISTINCT ON picks one row per order: the item with the highest price,
-- using it as a stand-in for "what this order was mainly about"
SELECT DISTINCT ON (oie.order_id)
    oie.order_id,
    oie.category AS primary_category
FROM order_items_enriched oie
ORDER BY oie.order_id, oie.price DESC;
...
-- Sanity check before moving to Python
SELECT COUNT(*) AS total_rows,
       COUNT(review_score) AS with_review,
       COUNT(delivery_days) AS with_delivery_days,
       COUNT(avg_distance_km) AS with_distance,
